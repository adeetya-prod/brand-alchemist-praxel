...
import { requestCreativeGeneration } from '@/app/actions/creatives'
...
export default function CreativeRequestForm({ brandId, brandName }: { brandId: string; brandName: string }) {
  ...
  const [format, setFormat] = useState<Format>('INSTAGRAM_SQUARE')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true)
    try {
      const result = await requestCreativeGeneration({ brandId, format, brief: brief || undefined })
      if ('error' in result) {
        setError('Failed to start generation. Please try again.')
        return
      }
      router.push(`/brands/${brandId}/creatives/${result.creativeId}`)
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <p className="text-xs text-gray-400 text-center">Generation typically takes 20–40 seconds.</p>
    </form>
  )
}
