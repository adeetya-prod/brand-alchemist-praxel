    ...
    <div className="space-y-4">
        ...
        {TONE_OPTIONS.map(keyword => {
          ...
          return (
            <button key={keyword} type="button" onClick={() => toggleTone(keyword)}
              className={`px-3 py-1.5 rounded-full text-sm font-medium border transition-colors ${selected ? 'bg-indigo-600 text-white border-indigo-600' : 'border-gray-300 text-gray-700 hover:border-indigo-400'}`}>
              {keyword}
            </button>
          )
        })}
      ...
      {values.voiceGuide && (
        <div className="mt-3 p-4 bg-indigo-50 rounded-lg border border-indigo-100">
          <p className="text-sm font-medium text-indigo-800 mb-1">Brand Voice Guide</p>
